 ...
 * 
 * Eitri_Rx: With GPS | With Bluetooth | With Multi-threading | POWDER-specific
 * 
...
#define READ_EQUILIBRIUM 10               // The number of samples to be read before we decide to use the IMU report

/* Servo PWM Control Values */
#define MAX_PWM 2150                       // Forward Motion (PWM ON pulse duration)
#define MID_PWM 1500                       // Stop Motion (PWM ON pulse duration)
#define MIN_PWM 850                        // Backward Motion (PWM ON pulse duration)
...
float tmpYaw = 0.0, requiredYawAngle = 0.0, currentYawAngle = 0.0;                  // The core yaw axis variables facilitating motor control
float tmpPitch = 0.0, requiredPitchAngle = 0.0, currentPitchAngle = 0.0;            // The core pitch axis variables facilitating motor control
...
boolean motorsInitiated = false;                                                    // A flag to indicate if the motors have been initiated or not
...
void control() {
  ...
  if ( (tmpYaw != requiredYawAngle) || (tmpPitch != requiredPitchAngle) ) {
...
TimedAction controlThread = TimedAction(10, control);                                                                 // A "proto-thread" for Servo control over Bluetooth Serial -- triggered every 10ms
...
/* The loop routine */
void loop() {
  /* Check in on the "proto-thread" */
  controlThread.check();

  /* The GPS data publish routine over Bluetooth Serial
