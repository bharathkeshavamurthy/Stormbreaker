...
class GPSEvent:
    ...
    """
    TODO: Update the "Satellites In View (SIV)" member, if needed
    """
    siv: int = 25
    fix_type: FixType = FixType.THREE_DIMENSIONAL
    carrier_solution_type: CarrierSolutionType = CarrierSolutionType.FLOAT_SOLUTION
    """
    TODO: Update these "members" with "component" values if you are conducting "field_work" and want to simulate the Tx
    """
...
def get_file_name_for_events(seq_number: int, key: str) -> str:
    ...
    """
    return ''.join(['../logs/simulator-logs/', key, '_', 'event_', str(seq_number), '.json'])
...
def gps_fetch_publish() -> None:
    ...
    try:
        kafka_producer = KafkaProducer(client_id='Pseudo_Heimdall_Tx', bootstrap_servers='xx.xx.xx.xx:9092', acks=1,
