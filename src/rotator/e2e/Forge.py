...
class SetupHandler(ABC):
    ...
    @abstractmethod
    def setup(self, clock_callback, mandates) -> None:
        """
         ...
         Args:
             clock_callback: The centralized clock callback in order to extract the synchronized and formatted UTC
                             timestamp
...
def pack_dict_into_dataclass(dict_: Dict, dataclass_: dataclass) -> dataclass:
    ...
    try:
        ...
        for k, v in dict_.items():
            # FIXME (v21.09): If secondary information tiers other than Member are used, come up with a better logic...
...
def gps_fetch_publish(clock_callback: Callable, serial_comm_config: SerialCommConfig, serial_comm_registry: Dict,
    ...
    Args:
        clock_callback: The centralized clock callback in order to extract the synchronized and formatted UTC timestamp
    ...
    try:
        ...
        while 1:
            ...
            """
            This check might be necessary if the data received over serial is "\b''" or some other non-compliant yet 
            standard string -- so, it does not make sense to throw NMEAValidationError.
            TODO (v21.09): Throw a NMEAValidationError, if needed in an application...
            """
            if not nmea_validate(nmea_gps_data, reduced):
                logger.warning('NMEAValidationError caught while parsing the NMEA GPS data received over serial at '
                               '{}'.format(logger.name))
                continue
            nmea_parse(clock_callback(), nmea_gps_data, gps_event, reduced)
    ...
    try:
        ...
        while 1:
            for msg in kafka_consumer:
                ...
                """
                This might seem weird: Receive JSON --> Pack into GPSEvent --> Print JSON--but, this is required to
                ensure homogeneity in logging in situations where we encounter different platforms and/or versions
                with different "pretty-print" or "indentation" values.
                """
...
def principal_axes_positioning(clock_callback: Callable, realm_type: RealmTypes, serial_comm_config: SerialCommConfig,
    ...
    try:
        ...
        with lock:
            ...
            if not uc.isOpen():
                uc.open()
        while 1:
            if not (tx_gps_data.is_gnss_fix_ok and rx_gps_data.is_gnss_fix_ok):
                logger.warning('No GNSS Fix for TxRealm and/or RxRealm GPS data | '
                               'Not initiating Principal Axes Positioning | '
                               'TxRealm GNSS Fix = {} | RxRealm GNSS Fix = {}'.format(tx_gps_data.is_gnss_fix_ok,
                                                                                      rx_gps_data.is_gnss_fix_ok))
                continue
            with lock:
                # Pull out the Tx and Rx GPS data for fixed analyses
            ...
            seq_number += 1
            imu_trace = IMUTrace(seq_number=seq_number, timestamp=clock_callback(),
