...
class TxSetupHandler(SetupHandler):
    ...
    def __init__(self, mobility: Mobility, serial_comm_config: SerialCommConfig, kafka_config: KafkaConfig):
        ...
        self.tx_gps_data: GPSEvent = GPSEvent()
        self.kafka_client: KafkaClient = KafkaClient(kafka_config)

    def setup(self, clock_callback: Callable, mandates: List[ControllerMandate]) -> None:
        """
        ...
        Args:
            clock_callback: The centralized clock callback in order to extract the synchronized and formatted UTC
                            timestamp from an NTPClient at the Centralized Realms Python Controller
            ...
            with ThreadPoolExecutor(max_workers=3 * DataTypes.__len__()) as executor:
                for mandate in mandates:
                    ...
                    if mandate.data_type.value == DataTypes.GPS.value:
                        ...
                        executor.submit(mandate.production_routine, clock_callback, self.serial_comm_config,
                                        self.serial_comm_registry, self.kafka_client,
                                        mandate.production_topic, self.tx_gps_data,
                                        LOGGER_Tx, True, lock)
                        ...
                        executor.submit(principal_axes_positioning, clock_callback, RealmTypes.TRANSMISSION,
...
class TxController(Controller):
    ...
    __serial_comm_config = SerialCommConfig(id='Bifrost_Tx', is_wireless=False, port='COM3',
    ...
    def __init__(self):
        ...
        try:
            if TxController.__controller is not None:
                ...
                with rpyc.connect(os.getenv(REALMS_HOSTNAME_ENVIRONMENT_VARIABLE, default='xx.xx.xx.xx'),
                    ...
                    # Create a callback to the Centralized Realms Python Controller's timestamp synchronization service
                    clock_callback = realms_connection.root.get_timestamp
                    # Post-registration, start the setup handler
                    self.setup_handler.setup(clock_callback, self.mandates(realms_connection, data_type_associations))
