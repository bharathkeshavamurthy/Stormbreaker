...
static bool private_write  (struct TinyPngOut this[static 1], const uint8_t data[], size_t len);
...
enum TinyPngOut_Status TinyPngOut_init(struct TinyPngOut *this, uint32_t w, uint32_t h, struct netconn *out) {
	// Check arguments
	if (w == 0 || h == 0 || out == NULL)
	...
	this->output = out;
	if (!private_write(this, header, sizeof(header) / sizeof(header[0])))
...
enum TinyPngOut_Status TinyPngOut_write(struct TinyPngOut *this, const uint8_t pixels[], size_t count) {
	if (count > SIZE_MAX / 3)
		return TINYPNGOUT_INVALID_ARGUMENT;
	...
	while (count > 0) {
		...
		if (this->positionX == 0) {  // Beginning of line - write filter method byte
			uint8_t b[] = {0};
			if (!private_write(this, b, sizeof(b) / sizeof(b[0])))
				return TINYPNGOUT_IO_ERROR;
			crc32(this, b, 1);
			adler32(this, b, 1);
			this->positionX++;
			...
			if (count < n)
				n = (uint16_t)count;
			assert(n > 0);
			if (!private_write(this, pixels, n))
				return TINYPNGOUT_IO_ERROR;
		...
		if (this->positionX == this->lineSize) {  // Increment line
			...
			if (this->positionY == this->height) {  // Reached end of pixels
				...
				if (!private_write(this, footer, sizeof(footer) / sizeof(footer[0])))
					return TINYPNGOUT_IO_ERROR;
			}
		}
	}
...
static bool private_write(struct TinyPngOut *this, const uint8_t data[], size_t len) {
	//return fwrite(data, sizeof(data[0]), len, this->output) == len;
	netconn_write(this->output, data, len, NETCONN_NOCOPY);
	return true;
}


// Reads the 'crc' field and updates its value based on the given array of new data.
static void crc32(struct TinyPngOut *this, const uint8_t data[], size_t len) {
	this->crc = ~this->crc;
...
static void adler32(struct TinyPngOut *this, const uint8_t data[], size_t len) {
