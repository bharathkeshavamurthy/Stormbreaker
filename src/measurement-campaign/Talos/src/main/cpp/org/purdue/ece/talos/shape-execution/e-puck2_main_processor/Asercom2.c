...
int run_asercom2(void) {
	...
	/* BK-2020: Simultaneous Bluetooth ASERCOM v2 and Magnetometer orientation/alignment capabilities */
	systime_t time;
	float mag_values[3];
					...
					case 0x8: // Get all sensors.
                    	...
                    	switch(magneto_state) {
							...
							case 1:
								...
								heading += 180.0;
								/* TODO: Improve the code to send "double" values to the PC BT controller code */
								buffer[i++] = (int)heading;
								buffer[i++] = (int)imu_values.magnetometer[0];
								for (int z = 0; z < 10; z++) {
									buffer[i++] = 0;
								}
