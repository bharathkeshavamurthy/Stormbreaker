...
class SerialComm {
	...
	public:
		...
		*/
		...
		/**
			...
			@return	returns the number of bytes correctly written to the serial port or -1 in case of error.
		*/
		int32_t writeData(char *buf, int num_bytes, int usleep_time);
		...
		/**
			...
			@return	returns the number of bytes correctly red from the device or -1 in case of error..
		*/
		int32_t readData(char *buf, int num_bytes, uint32_t usleep_time);
	...
	private:
        #if defined(_WIN32) || defined(_WIN64)
            HANDLE m_hCom;  /**< handle to the communication port.*/
        #else
