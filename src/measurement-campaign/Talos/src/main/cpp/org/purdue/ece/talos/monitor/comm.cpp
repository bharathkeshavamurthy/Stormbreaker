...
int TCommPort::WriteString(const char *outString)
{
    VerifyOpen();
...
int TCommPort::ReadBytes(BYTE *buffer, unsigned int MaxBytes, int usleepTime)
{

    VerifyOpen();
    int bytesRead=0;
    DWORD currentBytesRead=0;
    int timeout = usleepTime/1000;		//wait 100 useconds between every read (Bluetooth need 70usec/bytes); the loop lasts for usleep_time/70 times
    while(bytesRead<MaxBytes) {
        if(!ReadFile(m_hCom,&buffer[bytesRead],MaxBytes-bytesRead,&currentBytesRead,NULL))
            return -1;
        bytesRead+=currentBytesRead;
        if(bytesRead<MaxBytes) {	//wait to receive something new
            Sleep(1);				//1/(115200/8) = 70 useconds per byte
            timeout--;
        }
        if(timeout<=0) {
            break;
...
/*
    ...
    do{
        MaxBytes=MaxBytes-bytes_read;
        buffer=buffer+bytes_read ;                             //read until all asked byte are arrived
        if(!ReadFile(m_hCom,buffer,MaxBytes,&bytes_read,NULL))
            return -1;
        //        throw ECommError(ECommError::READ_ERROR);
    }
...
void TCommPort::DiscardBytes(unsigned int MaxBytes)
{
    ...
    ReadBytes(dummy, MaxBytes, 100000);
    delete []dummy;
}

int TCommPort::PurgeCommPort(void)
{
    VerifyOpen();
...
int TCommPort::FlushCommPort(void)
{
    VerifyOpen();
...
int TCommPort::PutByte(BYTE value)
{
    VerifyOpen();
