...
static THD_FUNCTION(selector_thd, arg)
{
    ...
    unsigned int group_factor, temporary_group_factor = 0;
	unsigned int binary_representation[5] = {0};
	double divisor = 11.25;
	double ans = 0.0;
	double weight = 0.0;
...
//							}
			...
			/* BK-2020: Magnetometer testing: Encode LED values based on the robot's heading */
			case 14:
				switch(magneto_state) {
					...
					case 1:
						...
						e_led_clear();
						ans = heading / divisor;
						group_factor = floor(ans);
						temporary_group_factor = group_factor;
						for (int i = 0; temporary_group_factor > 0; i++) {
							binary_representation[i] = temporary_group_factor % 2;
							temporary_group_factor = temporary_group_factor / 2;
						}
						for (int j = 0; j < 5; j++) {
							e_set_led(j, binary_representation[j]);
						}

						/* 4490 */
						//weight = ((imu_values.magnetometer[0] >= 14.5) && (imu_values.magnetometer[0] <= 17.5)) ? 0 : (16.0 - imu_values.magnetometer[0]);
						//right_motor_set_speed( floor( weight * 10 ) );
						//left_motor_set_speed( floor( weight * 10 * -1 ) );

						/* 4447 */
						//weight = ((imu_values.magnetometer[0] >= 21.0) && (imu_values.magnetometer[0] <= 33.0) && (group_factor >= 2) && (group_factor <= 5)) ? 0 : (3.44 - group_factor);
						//right_motor_set_speed( floor( weight * 10 ) );
						//left_motor_set_speed( floor( weight * 10 * -1 ) );

						/* if ((heading > 332.5) || (heading <= 27.5))
							e_set_led(0, 1);
						else if ((heading > 27.5) && (heading <= 72.5))
							e_set_led(1, 1);
						else if ((heading > 72.5)  && (heading <= 107.5))
							e_set_led(2, 1);
						else if (( heading > 107.5) && (heading <= 152.5))
							e_set_led(3, 1);
						else if (( heading > 152.5) && (heading <= 207.5))
							e_set_led(4, 1);
						else if ( (heading > 207.5) && (heading <= 252.5))
							e_set_led(5, 1);
						else if ( (heading > 252.5) && (heading <= 287.5))
							e_set_led(6, 1);
						else if ( (heading > 287.5) && (heading <= 332.5))
							e_set_led(7, 1);
				    	if (SDU1.config->usbp->state == USB_ACTIVE) {
