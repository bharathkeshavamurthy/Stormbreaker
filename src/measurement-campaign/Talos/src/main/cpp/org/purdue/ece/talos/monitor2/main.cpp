...
int main(int argc, char *argv[]) {
    ...
    QObject::connect(commThread, SIGNAL(connectionClosed()), &main, SLOT(disconnect()));
    QObject::connect(commThread, SIGNAL(cannotOpenPort(QString)), &main, SLOT(printMessage(QString)));
    QObject::connect(commThread, SIGNAL(updateUiState(uint8_t)), &main, SLOT(updateUiState(uint8_t)));
    QObject::connect(commThread, SIGNAL(updateFps()), &main, SLOT(updateFps()));
    QObject::connect(main.ui.btnUp, SIGNAL(clicked()), commThread, SLOT(goForward()));
    QObject::connect(main.ui.btnDown, SIGNAL(clicked()), commThread, SLOT(goBackward()));
