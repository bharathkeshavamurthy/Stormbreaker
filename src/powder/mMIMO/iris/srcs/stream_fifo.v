//----------------------------------------------------------------------
//-- AXI stream wrapper for the hard FIFO with arbitrary bit width
//--
...
// WARNING : When ENABLE_TUSER is set to 1, the first word of every
// burst (IN_TDATA + IN_TUSER) is loaded into the SRL-based IN_FIFO in
// one clock cycle, but transferred into the larger internal FIFO in two
// clock cycles.  This presents the possibility of data slowly building
// up in the IN_FIFO when multiple bursts are streamed in continuously
// without any gaps within them or between them.  For this reason, it
// is advisable to insert a single-cycle gap between the last cycle of
// one burst (cycle with IN_TLAST asserted) and the first cycle of the
// subsequent burst.  Failure to do this could cause the unexpected
// deasseetion of IN_TREADY, even though ALMOST_FULL was never asserted
...
module stream_fifo # (
    ...
    parameter PACKET_MODE = 0) //1=block output until complete packet buffered
(
    input wire clk,
    input wire rst,
    ...
    wire wr_rst_busy;
    wire rd_rst_busy;
    wire read = rfifo_tready && !empty && rd_enabled && !rd_rst_busy;
    wire write = wfifo_tvalid && !full && wr_enabled && !wr_rst_busy;
    assign wfifo_tready = !full && wr_enabled && !wr_rst_busy && in_wr_burst;
    assign rfifo_tvalid = !empty && rd_enabled && !rd_rst_busy && in_rd_burst;
    ...
    //maintain count of packets within the stream fifo
    reg [1:0] pkt_count;
    wire wlast = wfifo_tvalid && wfifo_tready && wfifo_tlast;
    wire rlast = rfifo_tvalid && rfifo_tready && rfifo_tlast;
    always @(posedge clk) begin
        if (rst) pkt_count <= 2'b0;
        else if (wlast && !rlast) pkt_count <= pkt_count + 1'b1;
        else if (!wlast && rlast) pkt_count <= pkt_count - 1'b1;
    end

    generate
    if (PACKET_MODE) begin
        //write until one packet is streaming out and other complete one is in the fifo
        assign wr_enabled = pkt_count < 2'h2; //write for packet counts of 0 and 1
        assign rd_enabled = pkt_count != 2'h0; //read when one packet is present
    end
    else begin
        assign wr_enabled = 1'b1;
        assign rd_enabled = 1'b1;
    end
    endgenerate
