...
//! @brief 				Fast fixed-point arithmetic for 64-bit numbers. Be
//! careful of overflow!
//! @details
...
//! @brief		Perform a fixed point multiplication without a 128-bit
//! intermediate result.
//!	@warning	This is fast but beware of intermediary overflow!
template <uint8_t p> inline int64_t FixMulF(int64_t a, int64_t b) {
  // Rule with fixed-point multiplication, you have
  // to right-shift result by the precision.
  return (a * b) >> p;
}

//! @brief		Perform a fixed point division without a 128-bit intermediate
//! result.
//! @details	No loss of precision since shift is done before division.
//!	@warning	This is fast but beware of intermediary overflow!
template <uint8_t p> inline int64_t FixDiv(int64_t a, int64_t b) {
...
//! @details	Do not write "myFpNum = FloatToRawFix64()"! This function
//! outputs a raw
//!				number, so you would have to use the syntax
//!"myFpNum.rawVal = FloatToRawFix64()".
//! @warning	Slow!
template <uint8_t p> int64_t FloatToRawFix64(float f) {
  return (int64_t)(f * (1 << p));
}

//! @brief		Converts from float to a raw fixed-point number.
//! @details	Do not write "myFpNum = DoubleToRawFix64()"! This function
//! outputs a raw
//!				number, so you would have to use the syntax
//!"myFpNum.rawVal = DoubleToRawFix64()".
//! @warning	Slow!
template <uint8_t p> int64_t DoubleToRawFix64(double f) {
...
public:
  ...
  //!					to 64-bit before shifting, otherwise
  //!truncation
  //!					will occur.
  ...
  Fp64f(float f) : rawVal(FloatToRawFix64<p>(f)) {
    // nothing
  }
  Fp64f(double f) : rawVal(FloatToRawFix64<p>((double)f)) {
    // nothing
  }
  ...
  bool operator==(Fp64f r) const { return rawVal == r.rawVal; }

  bool operator!=(Fp64f r) const { return !(*this == r); }
  ...
  operator float() { return (float)rawVal / (float)(1 << p); }

  //! @brief		Conversion operator from fixed-point to double.
  //! @note		Similar to float conversion.
  operator double() { return (double)rawVal / (double)(1 << p); }
